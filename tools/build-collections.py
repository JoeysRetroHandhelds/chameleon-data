...
def _auto_short(rule: dict, resolvers, hltb) -> list[dict]:
    """Every game under `maxHours` on the named platforms, capped at `limit` per platform."""
    max_hours = rule.get("maxHours", 3)
...
def main():
    ...
    for rule in sources.get("auto", []):
        print(f"auto: {rule['name']}")
        games = _auto_short(rule, resolvers, hltb) if rule.get("kind") == "short" else []
        if games:
            collections.append(
                {
                    "id": rule["id"],
